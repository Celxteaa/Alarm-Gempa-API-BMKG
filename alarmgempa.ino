...
void bunyikanSirineBencana(int durasiDetik) {
  Serial.println("ALARM AKTIF: BUNYI PANJANG CONTINUOUS (SENSING STYLE)");
  
  // LED Bawaan NYALA (LOW pada NodeMCU V3)
  digitalWrite(LED_BUILTIN, LOW); 
  
  // Nyalakan buzzer langsung (2700 Hz)
  tone(BUZZER_PIN, 2700); 
  
  // Tahan bunyi sepanjang durasi yang diminta (misal 5 atau 15 detik)
  delay(durasiDetik * 1000); 
  
  // Matikan total setelah durasi selesai
  noTone(BUZZER_PIN); 
  digitalWrite(LED_BUILTIN, HIGH); // LED Bawaan MATI
}
...
void tampilkanKeOLED(String status, String info1, String info2) {
  ...
  // LOGIKA EKSTRAKSI JAM: Mengambil jam dari idGempaTerakhir (Format: Tanggal_Jam)
  if (idGempaTerakhir != "") {
    int posisiUnderscore = idGempaTerakhir.indexOf('_');
    if (posisiUnderscore != -1) {
      // Mengambil 5 karakter setelah underscore (Format HH:MM, contoh: 14:25)
      String jamMenit = idGempaTerakhir.substring(posisiUnderscore + 1, posisiUnderscore + 6);
      
      // Set posisi di pojok kanan atas (koordinat X=95, Y=0)
      display.setCursor(95, 0); 
      display.print(jamMenit); 
    }
  }
...
void setup() {
  ...
  tampilkanKeOLED("ONLINE", "Sistem Siap", "Mengambil data BMKG...");
  delay(2000);
}
